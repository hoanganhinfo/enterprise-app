...
public class QcInspectionController implements Controller, InitializingBean {
	...
	public ModelAndView handleRenderRequest(RenderRequest request,
		...
		JSONArray aStatusType = new JSONArray();
		JSONArray aTransType = new JSONArray();
		JSONArray aDefectLevel = new JSONArray();
		List<StatusType> statusList = StatusType.getAllQCInspector();
		for (StatusType statusType : statusList) {
			jsonOrg = new JSONObject();
			jsonOrg.put("value", statusType.getValue()+"");
			jsonOrg.put("name", statusType.getLabel());
			aStatusType.add(jsonOrg);
		}
		List<TransType> transTypeList = TransType.getAllQCTransType();
		for (TransType transType : transTypeList) {
			jsonOrg = new JSONObject();
			jsonOrg.put("value", transType.getValue());
			jsonOrg.put("name", transType.getLabel());
			aTransType.add(jsonOrg);
		}
		List<PriorityType> defectLevelList = PriorityType.getDefectLevelList();
		for (PriorityType defectLevel : defectLevelList) {
			jsonOrg = new JSONObject();
			jsonOrg.put("value", defectLevel.getValue());
			jsonOrg.put("name", defectLevel.getLabel());
			aDefectLevel.add(jsonOrg);
		}
...
//		jsonOrg = new JSONObject();
		...
		map.put("statusTypeJsonData", aStatusType);
		map.put("transTypeJsonData", aTransType);
		map.put("defectLevelJsonData", aDefectLevel);
		
		if (themeDisplay.isSignedIn()) {
			return new ModelAndView("app/QcInspector", "param", map);
		}
